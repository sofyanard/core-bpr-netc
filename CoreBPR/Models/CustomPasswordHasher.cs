...
namespace CoreBPR.Models
{
    public class CustomPasswordHasher
    {
        ...
        public string HashPassword(string password)
        {
            ...
            {
                var pbkdf2 = new Rfc2898DeriveBytes(password, salt, _iterCount, _hashAlgorithmName);
        ...
        public bool VerifyPassword(string hashedPassword, string enteredPassword)
        {
            ...
            try
            {
                if (_includeHeaderInfo)
                {
                    ...
                    var verifyPrf = KeyDerivationPrf.HMACSHA256;
                    switch (shaUInt)
                    {
                        case 0:
                            verifyPrf = KeyDerivationPrf.HMACSHA1;
                            break;
                        case 1:
                            verifyPrf = KeyDerivationPrf.HMACSHA256;
                            break;
                        case 2:
                            verifyPrf = KeyDerivationPrf.HMACSHA512;
                            break;
                        default:
                            verifyPrf = KeyDerivationPrf.HMACSHA256;
                            break;
                    ...
                    var verifyAlgorithmName = HashAlgorithmName.SHA256;
                    switch (shaUInt)
                    {
                        case 0:
                            verifyAlgorithmName = HashAlgorithmName.SHA1;
                            break;
                        case 1:
                            verifyAlgorithmName = HashAlgorithmName.SHA256;
                            break;
                        case 2:
                            verifyAlgorithmName = HashAlgorithmName.SHA512;
                            break;
                        default:
                            verifyAlgorithmName = HashAlgorithmName.SHA256;
                            break;
                ...
                {
                    var pbkdf2 = new Rfc2898DeriveBytes(enteredPassword, salt, _iterCount, _hashAlgorithmName);
